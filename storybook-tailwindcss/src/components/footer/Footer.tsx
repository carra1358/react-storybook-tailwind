...
export const Footer = ({logo}: FooterProps) => {
    ...
    const Link = ({label,icon}:LinkProps) => {
      return (
        icon? 
        <li><a href="#" className=" m-4 lg:m-0"><img src={icon} className=" w-5 h-5"/><span className="sr-only">{label}</span></a></li> 
        : <li><a href="#" className=" capitalize text-white font-semibold ">{label}</a></li>)
      
    }

    return (
        <div className=" bg-primary py-5 px-6 text-fontXS leading-8">
         <div className="lg:flex ">
         <div>
            <img src={logo} alt="" />
         </div>
         <div className="lg:flex lg:justify-between  lg:w-5/6 lg:px-32" >
            <ul >
                <li className="text-smallHeader uppercase mt-10 lg:mt-0">Community</li>
